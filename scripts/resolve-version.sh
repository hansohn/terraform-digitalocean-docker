...
#
# Only the DigitalOcean tooling layered on top of the base terraform image is
...
case "${TOOL}" in
  doctl)
    VERSION=$(${CURL} "https://api.github.com/repos/digitalocean/doctl/releases/latest" | jq -r .tag_name | sed -e "s:^v::") ;;
  *)
